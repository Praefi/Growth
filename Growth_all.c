...
int main (void) {
	...
	Spielfeld Sf_opague, numbers_of_, stack_of_;	//ruck muss raus, done, Sf_opague==Opague-Spielfeld, numbers_of_==statistics, number_of_[geben][0]==Matchballs etc., stack_of_==stack of tac
	personal_Spielfeld Field, Sf_nl_, Sf_od_, Field_journey;	//cons_field in Field, cons_journey in Field_journey, Field[0]==Das Spielfeld, Field_journey[0]==Das Journeyfeld
	...
	*/
	...
	while (playtime != 0){		// Außenschleife für mehrere Spiele hintereinander
		...
		for (unsigned int p=0; p<=AOP; p+=1){	//Initializise after number_of_players, go on
			...
			if (geben == 1) {
				...
				if (ttt != 0) {		//Strafen wegen der Zeit
					if ((pere[geben] != 0)&&(warning_system <= 5)&&((pere[geben]+warning_system) == 7)) {
						...
						Sf_od_[geben] = old_dying (Field[0], m, n, d, e, gamemode, information_code, geben);
						Field[0] = change (Field[0], Sf_nl_[geben], Sf_od_[geben], m, n, gamemode, number_of_players, ges, geben); 	//Change befreit
						g += 1;
						printf(" \n ");
						printf("	You got a penalty. \n ");
						printf(" \n ");
				...
				if (var_[geben%number_of_players+1] == 1010){		//geben%number_of_players+1 ist der Nachfolger von geben.
					if (tac != 0) {
						if (number_[geben] <= 5){
							numbers_of_[geben][number_[geben]] += 1;
						} else if (number_[geben] == 6){
							numbers_of_[geben][0] += 1;
						}
					} else if (opt == 5) {
						numbers_of_[geben][use_number] -= 1;
					} else {
						numbers_of_[geben][number_[geben]] -= 1;
					}
				...
				if ((tac != 0)&&((numbers_of_[geben][1]+numbers_of_[geben][2]+numbers_of_[geben][3]+numbers_of_[geben][4]+numbers_of_[geben][5]+numbers_of_[geben][0]) == 0)){
					for (unsigned int p=0; p<=5; p+=1){
						numbers_of_[geben][p] = tac;
					}
				}
				...
				while (menuoperator != 0) {		//Beginn des Zuges
					...
					if (tac != 0){
						printf("	Your stack of numbers:	\n	1) [%u],		2) [%u],		3) [%u], \n\n	4) [%u],		5) [%u],		6) [%u],\n", numbers_of_[geben][1], numbers_of_[geben][2], numbers_of_[geben][3], numbers_of_[geben][4], numbers_of_[geben][5], numbers_of_[geben][0]);
						printf("\n");
						printf("\n");
					}
					...
					if ((tac != 0)&&(var_[geben] <= 62)&&(var_[geben] > 10)){
						...
						if (((var_[geben]%10) > 2)||((var_[geben]%10) == 0)){
							...
							if (numbers_of_[geben][tac_controll] == 0){
								var_[geben] = 7;
								menuoperator = 1;
							} else {
								numbers_of_[geben][tac_controll] -= 1;		//Die tac-Abrechnung
							}
						}
				...
				if (var_[geben] == 0){
					...
					if (var_[geben] == 100){		//1, 2, 3,... durch "geben" ersetzen, go on
						...
						if (var_[geben] < 50){
							...
							if (var_[geben] == 11){numbers_of_[geben][1] += 1; Plus (m, n, geben, Field[0], limit_at_all, gamemode, information_code, number_of_players, Colored, opague, Sf_opague);}
							if (var_[geben] == 12){numbers_of_[geben][1] += 1; Minus (m, n, geben, Field[0], gamemode, information_code, Colored, opague, Sf_opague);}
							if (var_[geben] == 21){numbers_of_[geben][2] += 1; Move (m, n, geben, Field[0], gamemode, information_code, Colored, opague, Sf_opague);}
							if (var_[geben] == 41){numbers_of_[geben][4] += 1; Move (m, n, geben, Field[0], gamemode, information_code, Colored, opague, Sf_opague);}
							if (var_[geben] == 22){numbers_of_[geben][2] += 1; Change (m, n, geben, Field[0], gamemode, information_code, Colored, opague, Sf_opague, position);}
							if (var_[geben] == 31){numbers_of_[geben][3] += 1; Destroy (m, n, geben, Field[0], gamemode, information_code, Colored, opague, Sf_opague);}
							if (var_[geben] == 42){
								numbers_of_[geben][4] += 1;
							...
							if (var_[geben] == 51){numbers_of_[geben][5] += 1; w += 1;}		//w
							if (var_[geben] == 52){numbers_of_[geben][5] += 1; w -= 1;}		//w
							if (var_[geben] == 61){numbers_of_[geben][6] += 1; e += 1;}		//e
							if (var_[geben] == 62){numbers_of_[geben][6] += 1; if (d != 0) {d -= 1;} else {d = 10;}}		//d
						}
						...
						Sf_nl_[geben] = new_life (Field[0], m, n, w, gamemode, information_code, geben);
						...
						Sf_od_[geben] = old_dying (Field[0], m, n, d, e, gamemode, information_code, geben);	// , geben)
						...
						if (var_[geben] == 32){numbers_of_[geben][3] += 1; Revive(m, n, Sf_od_[geben], Field[0]);}	//, geben)
						...
						Field[0] = change (Field[0], Sf_nl_[geben], Sf_od_[geben], m, n, gamemode, number_of_players, ges, geben); 	//Change befreit, ,geben)
							...
							while (sequence[0][0] == 100) {
								...
								if ((sequence[0][0] >= seq_max)||(sequence[geben][sequence[0][0]] == 0)) {
									...
									for (unsigned int p=1; p<=sequence[0][0]; p+=1) {		//Index?
										...
										Sf_nl_[geben] = new_life (Field[0], m, n, w, gamemode, information_code, geben);		//let Index go over it, even without reason? What will Index do? Nothing, i guess...
										Sf_od_[geben] = old_dying (Field[0], m, n, d, e, gamemode, information_code, geben);
										Field[0] = change (Field[0], Sf_nl_[geben], Sf_od_[geben], m, n, gamemode, number_of_players, ges, geben); 	//Change befreit
				...
				while (menuoperator != 0) {
					...
					if ((gamemode == 6)&&(((scwhp == 1)&&((opt == 0)||((use_number == 2)||(use_number == 3)||(use_number == 4)))&&((tac == 0)||((numbers_of_[geben][2] != 0)||(numbers_of_[geben][3] != 0)||(numbers_of_[geben][4] != 0))))||(scwhp == 2))) {
								...
								if (suprise == 0) {
									for (unsigned int i=1; i<m-1; i+=1){
										for (unsigned int j=1; j<n-1; j+=1){
											...
											if (journey == 1){
												cons_journey_3[i][j] = Field_journey[0][i][j];
											}
										}
									}
									sup_num = use_number;
									...
									if (lim == 1){
										...
										for (unsigned int i=1; i<m-1; i+=1){
											for (unsigned int j=1; j<n-1; j+=1){
												...
												if (journey == 1){
													Field_journey[0][i][j] = cons_journey_3[i][j];
												}
											}
										}
										use_number = sup_num;
					...
					if ((tac != 0)&&(var_[3] <= 62)&&(var_[3] > 10)){
						...
						if (((var_[3]%10) > 2)||((var_[3]%10) == 0)){
							...
							if (numbers_of_[geben][tac_controll] == 0){
								...
								numbers_of_[geben][tac_controll] -= 1;
							}
						}
					...
					if (var_[3] == 100){		//gamemode 6 modify, done
						...
						for (unsigned int i=1; i<m-1; i+=1){
							for (unsigned int j=1; j<n-1; j+=1){
								...
								if (journey == 1){
									Field_journey[0][i][j] = cons_journey_2[i][j];
								}
							}
						}
						...
						if (journey == 1){
							...
							for (unsigned int i=1; i<m-1; i+=1){
								for (unsigned int j=1; j<n-1; j+=1){
									if ((Sf_od_3[i][j] == 303) && (Field_journey[0][i][j] == 0) && (journey_max >= limit_at_all)){
								...
								if (suprise == 0) {
									for (unsigned int i=1; i<m-1; i+=1){
										for (unsigned int j=1; j<n-1; j+=1){
											...
											if (journey == 1){
												cons_journey_4[i][j] = Field_journey[0][i][j];
											}
										}
									}
									sup_num = use_number;
									...
									if (lim == 1){
										...
										for (unsigned int i=1; i<m-1; i+=1){
											for (unsigned int j=1; j<n-1; j+=1){
												...
												if (journey == 1){
													Field_journey[0][i][j] = cons_journey_4[i][j];
												}
											}
										}
										use_number = sup_num;
				...
				if (var_[4] == 0){
					...
					if (var_[4] == 100){		//gamemode 6 modify, done
						...
						if (journey == 1){
							...
							for (unsigned int i=1; i<m-1; i+=1){
								for (unsigned int j=1; j<n-1; j+=1){
									if ((Sf_od_4[i][j] == 404) && (Field_journey[0][i][j] == 0) && (journey_max >= limit_at_all)){
										for (unsigned int u=(m-2); u>0; u-=1){
											for (unsigned int o=1; o<(n-1); o+=1){
								...
								if (suprise == 0) {
									for (unsigned int i=1; i<m-1; i+=1){
										for (unsigned int j=1; j<n-1; j+=1){
											...
											if (journey == 1){
												cons_journey_5[i][j] = Field_journey[0][i][j];
											}
										}
									}
									sup_num = use_number;
									...
									if (lim == 1){
										...
										for (unsigned int i=1; i<m-1; i+=1){
											for (unsigned int j=1; j<n-1; j+=1){
												...
												if (journey == 1){
													Field_journey[0][i][j] = cons_journey_5[i][j];
												}
											}
										}
										use_number = sup_num;
				...
				if (var_[5] == 0){
					...
					if (var_[5] == 100){		//gamemode 6 modify, done
						...
						if (journey == 1){
							...
							for (unsigned int i=1; i<m-1; i+=1){
								for (unsigned int j=1; j<n-1; j+=1){
									if ((Sf_od_5[i][j] == 505) && (Field_journey[0][i][j] == 0) && (journey_max >= limit_at_all)){
										for (unsigned int u=(m-2); u>0; u-=1){
											for (unsigned int o=1; o<(n-1); o+=1){
								...
								if (suprise == 0) {
									for (unsigned int i=1; i<m-1; i+=1){
										for (unsigned int j=1; j<n-1; j+=1){
											...
											if (journey == 1){
												cons_journey_6[i][j] = Field_journey[0][i][j];
											}
										}
									}
									sup_num = use_number;
									...
									if (lim == 1){
										...
										for (unsigned int i=1; i<m-1; i+=1){
											for (unsigned int j=1; j<n-1; j+=1){
												...
												if (journey == 1){
													Field_journey[0][i][j] = cons_journey_6[i][j];
												}
											}
										}
										use_number = sup_num;
				...
				if (var_[6] == 0){
					...
					if (var_[6] == 100){		//gamemode 6 modify, done
						...
						if (journey == 1){
							...
							for (unsigned int i=1; i<m-1; i+=1){
								for (unsigned int j=1; j<n-1; j+=1){
									if ((Sf_od_6[i][j] == 606) && (Field_journey[0][i][j] == 0) && (journey_max >= limit_at_all)){
										for (unsigned int u=(m-2); u>0; u-=1){
											for (unsigned int o=1; o<(n-1); o+=1){
								...
								if (suprise == 0) {
									for (unsigned int i=1; i<m-1; i+=1){
										for (unsigned int j=1; j<n-1; j+=1){
											...
											if (journey == 1){
												cons_journey_7[i][j] = Field_journey[0][i][j];
											}
										}
									}
									sup_num = use_number;
									...
									if (lim == 1){
										...
										for (unsigned int i=1; i<m-1; i+=1){
											for (unsigned int j=1; j<n-1; j+=1){
												...
												if (journey == 1){
													Field_journey[0][i][j] = cons_journey_7[i][j];
												}
											}
										}
										use_number = sup_num;
				...
				if (var_[7] == 0){
					...
					if (var_[7] == 100){		//gamemode 6 modify, done
						...
						if (journey == 1){
							...
							for (unsigned int i=1; i<m-1; i+=1){
								for (unsigned int j=1; j<n-1; j+=1){
									if ((Sf_od_7[i][j] == 707) && (Field_journey[0][i][j] == 0) && (journey_max >= limit_at_all)){
										for (unsigned int u=(m-2); u>0; u-=1){
											for (unsigned int o=1; o<(n-1); o+=1){
								...
								if (suprise == 0) {
									for (unsigned int i=1; i<m-1; i+=1){
										for (unsigned int j=1; j<n-1; j+=1){
											...
											if (journey == 1){
												cons_journey_8[i][j] = Field_journey[0][i][j];
											}
										}
									}
									sup_num = use_number;
									...
									if (lim == 1){
										...
										for (unsigned int i=1; i<m-1; i+=1){
											for (unsigned int j=1; j<n-1; j+=1){
												...
												if (journey == 1){
													Field_journey[0][i][j] = cons_journey_8[i][j];
												}
											}
										}
										use_number = sup_num;
				...
				if (var_[8] == 0){
					...
					if (var_[8] == 100){		//gamemode 6 modify, done
						...
						if (journey == 1){
							...
							for (unsigned int i=1; i<m-1; i+=1){
								for (unsigned int j=1; j<n-1; j+=1){
									if ((Sf_od_8[i][j] == 808) && (Field_journey[0][i][j] == 0) && (journey_max >= limit_at_all)){
										for (unsigned int u=(m-2); u>0; u-=1){
											for (unsigned int o=1; o<(n-1); o+=1){
								...
								if (suprise == 0) {
									for (unsigned int i=1; i<m-1; i+=1){
										for (unsigned int j=1; j<n-1; j+=1){
											...
											if (journey == 1){
												cons_journey_9[i][j] = Field_journey[0][i][j];
											}
										}
									}
									sup_num = use_number;
									...
									if (lim == 1){
										...
										for (unsigned int i=1; i<m-1; i+=1){
											for (unsigned int j=1; j<n-1; j+=1){
												...
												if (journey == 1){
													Field_journey[0][i][j] = cons_journey_9[i][j];
												}
											}
										}
										use_number = sup_num;
				...
				if (var_[9] == 0){
					...
					if (var_[9] == 100){		//gamemode 6 modify, done
						...
						if (journey == 1){
							...
							for (unsigned int i=1; i<m-1; i+=1){
								for (unsigned int j=1; j<n-1; j+=1){
									if ((Sf_od_9[i][j] == 909) && (Field_journey[0][i][j] == 0) && (journey_max >= limit_at_all)){
										for (unsigned int u=(m-2); u>0; u-=1){
											for (unsigned int o=1; o<(n-1); o+=1){
...
Spielfeld Spielfeld_Create (unsigned int m, unsigned int n) {		//calloc in malloc geändert und zurückgenommen (16.06.2018),
	Spielfeld Spiel;
	Spiel = calloc(m, sizeof(unsigned int*));
	for (unsigned int i=0; i<m; i+=1) {
		Spiel[i] = calloc(n, sizeof(unsigned int));
	}
	return Spiel;
}
...
void old_dying (Spielfeld Field[0], unsigned int m, unsigned int n, unsigned int d, unsigned int e, unsigned int gamemode, unsigned int* information_code, Spielfeld Sf_od_[geben]){
	Spielfeld Sf_temp;
	unsigned int a, e_down, d_up;
	
	Sf_temp = Spielfeld_Create (m, n);
	a = 0;
	e_down = 0;
	d_up = 0;
			...
			if (Field[0][i][j] == geben) {
				...
				if (gamemode != 9) {
					if ((a < d+1) || (a > e+1)){		//d=2, d=1, e=3, e=4
						Sf_temp[i][j] = 101*geben;		//Code für das Eleminieren
					}
	...
	for (unsigned int i=1; i<m-1; i+=1){
		for (unsigned int j=1; j<n-1; j+=1){
			if (Sf_temp[i][j] = 101*geben) {
				Sf_od_[geben][i][j] = Sf_temp[i][j];
			}
		}
	}
}

void change (Spielfeld Field[0], Spielfeld Sf_nl_[geben], Spielfeld Sf_od_[geben], unsigned int m, unsigned int n, unsigned int gamemode, unsigned int number_of_players, unsigned int* ges, unsigned int geben){	
...
void Vorganger (unsigned int geben, unsigned int number_of_players) {
	unsigned int vorher;
	vorher = 0;
...
void Revive (unsigned int m, unsigned int n, Spielfeld Sf_od_[geben], Spielfeld Field[0], geben){
	unsigned int a;
	Spielfeld temp;
	...
	for (unsigned int i=1; i<m-1; i+=1){
		for (unsigned int j=1; j<n-1; j+=1){
			if ((Sf_od_[geben][i][j] == 101*geben) && (temp[i][j] == 0)){
				Sf_od_[geben][i][j] = 0;
			}
		}
	}
					...
					for (unsigned int j=1; j<n-1; j+=1){
						if (Sf_nl_[geben][i][j] == 1){
							...
							for (unsigned int h=i-1; h<=i+1; h+=1){		//Regeneriert ihr Tod neue Zellen?
								for (unsigned int k=j-1; k<=j+1; k+=1){
									if ((h>0)&&(h<(m-1))&&(k>0)&&(k<(n-1))){
										if ((((Field[0][h][k] == 0) || ((Field[0][h][k] == 1)&&(Sf_od_[geben][h][k] == 101*geben))) && (Sf_nl_[geben][h][k] == 0))&&(Field[0][h][k] != 11)){
											for (unsigned int u=h-1; u<=h+1; u+=1){
												for (unsigned int o=k-1; o<=k+1; o+=1){
													if ((u>0)&&(u<(m-1))&&(o>0)&&(o<(n-1))){
														if ((Sf_nl_[geben][u][o] == 1) || (((Field[0][u][o] == 1)||(Field[0][u][o] == 11)) && (Sf_od_[geben][u][o] == 0))){
